...
// api_key:
// auth_token:
// tnid:         The ID of the tenant to update the theme for.
//
// Returns
// -------
// <rsp stat='ok' />
...
function ciniki_web_privateThemeUpdate(&$ciniki) {
    ...
    $rc = ciniki_web_checkAccess($ciniki, $args['tnid'], 'ciniki.web.privateThemeUpdate'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    ...
    //
    $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.web.theme', $args['theme_id'], $args);
    if( $rc['stat'] != 'ok' ) {
    ...
    foreach($valid_settings as $field) {
        if( isset($ciniki['request']['args'][$field]) ) {
            if( isset($settings[$field]['detail_value']) && $settings[$field]['detail_value'] != $ciniki['request']['args'][$field] ) {
                ...
                // Update the setting
                //
                $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.web.theme_setting', $settings[$field]['id'],
                ...
                // Add the setting
                //
                $rc = ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.web.theme_setting', 
