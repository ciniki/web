...
function ciniki_web_collectionDelete(&$ciniki) {
    ...
    $rc = ciniki_web_checkAccess($ciniki, $args['business_id'], 'ciniki.web.collectionDelete'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    ...
    $rc = ciniki_core_objectGetSubs($ciniki, $args['business_id'], 'ciniki.web.collection', $args['collection_id'], 'ciniki.web.collection_objref');
    if( $rc['stat'] != 'ok' ) {
    ...
    if( isset($rc['refs']) ) {
        ...
        foreach($refs as $rid => $ref) {
            $rc = ciniki_core_objectDelete($ciniki, $args['business_id'], 'ciniki.web.collection_objref', $ref['ref']['id'], NULL, 0x04);       
        }
    }
    ...
    // Delete the object
    //
    $rc = ciniki_core_objectDelete($ciniki, $args['business_id'], 'ciniki.web.collection', $args['collection_id'], NULL, 0x04);
    if( $rc['stat'] != 'ok' ) {
        $rc = ciniki_core_dbTransactionRollback($ciniki, 'ciniki.web');
        return $rc;
    }
