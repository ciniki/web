...
function ciniki_web_sliderImageUpdate(&$ciniki) {
    ...
    $rc = ciniki_web_checkAccess($ciniki, $args['tnid'], 'ciniki.web.sliderImageUpdate'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    ...
    $rc = ciniki_core_objectUpdate($ciniki, $args['tnid'], 'ciniki.web.slider_image', $args['slider_image_id'], $args, 0x04);
    if( $rc['stat'] != 'ok' ) {
        return $rc;
    }
    ...
    if( isset($args['sequence']) ) {
        ...
        $rc = ciniki_web_sliderUpdateSequences($ciniki, $args['tnid'], 
