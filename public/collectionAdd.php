...
function ciniki_web_collectionAdd(&$ciniki) {
    ...
    $rc = ciniki_web_checkAccess($ciniki, $args['tnid'], 'ciniki.web.collectionAdd'); 
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    } 
    ...
    $rc = ciniki_core_objectAdd($ciniki, $args['tnid'], 'ciniki.web.collection', $args, 0x07);
    if( $rc['stat'] != 'ok' ) {
    ...
    $rc = ciniki_web_collectionObjSettingsUpdate($ciniki, $args['tnid'], $collection_id, $ciniki['request']['args']);
    if( $rc['stat'] != 'ok' ) {
        ciniki_core_dbTransactionRollback($ciniki, 'ciniki.web');
        return $rc;
    }
