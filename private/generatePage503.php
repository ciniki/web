...
// -------
//
function ciniki_web_generatePage503($ciniki, $settings, $errors) {
...
//  header("HTTP/1.0 503 Not Found");
    header("Status: 503 Server Maintenance", true, 503);
    ciniki_core_loadMethod($ciniki, 'ciniki', 'web', 'private', 'generatePageHeader');
    $rc = ciniki_web_generatePageHeader($ciniki, $settings, 'Maintenance', array());
    if( $rc['stat'] != 'ok' ) { 
        return $rc;
    }
    $content .= $rc['content'];
    ...
    if( (isset($settings['theme']['header-article-title']) && $settings['theme']['header-article-title'] == 'yes')
        ...
        if( isset($settings['theme']['header-article-title']) && $settings['theme']['header-article-title'] == 'yes' ) {
            $content .= "<h1 class='page-header-title'>503</h1>";
        }
