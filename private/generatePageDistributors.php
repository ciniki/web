...
// -------
//
function ciniki_web_generatePageDistributors($ciniki, $settings) {
    ...
    $article_title = 'Distributors';
    if( isset($settings['page-distributors-name']) && $settings['page-distributors-name'] != '' ) {
        $page_title = $settings['page-distributors-name'];
    }
    $base_url = $ciniki['request']['base_url'] . '/distributors';
...
/*  if( isset($ciniki['request']['uri_split'][0]) && $ciniki['request']['uri_split'][0] == 'category' 
        ...
        $article_title = "<a href='" . $ciniki['request']['base_url'] . "/distributors'>$page_title</a>";
        if( $rc['tag_name'] != '' ) {
        ...
        if( count($distributors) > 0 ) {
            ...
            $base_url = $ciniki['request']['base_url'] . "/distributors";
            $rc = ciniki_web_processCIList($ciniki, $settings, $base_url, $distributors, array('notitle'=>'yes'));
            if( $rc['stat'] != 'ok' ) {
                return $rc;
            }
            $page_content .= $rc['content'];
        } else {
            $page_content .= "<p>We're sorry, but there doesn't appear to be any distributors in this category.</p>";
        }
...
*/
    ...
    if( isset($ciniki['request']['uri_split'][0]) 
        ...
        $display_profile = 'yes';
        $distributor_permalink = $ciniki['request']['uri_split'][0];
        $article_title = '';
    ...
    elseif( isset($ciniki['request']['uri_split'][0]) 
        ...
        $distributor_permalink = $ciniki['request']['uri_split'][2];
    ...
    elseif( isset($ciniki['request']['uri_split'][0]) 
        ...
        $distributor_permalink = $ciniki['request']['uri_split'][4];
    ...
    elseif( isset($ciniki['request']['uri_split'][0]) 
        ...
        $article_title = "<a href='$base_url'>Distributors</a>";
    ...
    if( $display_list == 'yes' && isset($distributors) ) {
        if( count($distributors) > 0 ) {
            ...
            $page_content .= "<p>No distributors found for this area.</p>";
        }
    }
