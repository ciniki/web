...
function ciniki_web_generatePageExhibitors($ciniki, $settings) {
	...
	if( isset($ciniki['request']['uri_split'][0]) && $ciniki['request']['uri_split'][0] != '' 
		...
		if( $rc['stat'] != 'ok' ) {
			return array('stat'=>'404', 'err'=>array('pkg'=>'ciniki', 'code'=>'1300', 'msg'=>"I'm sorry, but we can't seem to find the image your requested.", $rc['err']));
		}
		$participant = $rc['participant'];

		if( !isset($participant['images']) || count($participant['images']) < 1 ) {
			return array('stat'=>'404', 'err'=>array('pkg'=>'ciniki', 'code'=>'490', 'msg'=>"I'm sorry, but we can't seem to find the image your requested."));
		}
	...
	elseif( isset($ciniki['request']['uri_split'][0]) && $ciniki['request']['uri_split'][0] != '' ) {
		...
		$rc = ciniki_exhibitions_web_participantDetails($ciniki, $settings, 
			...
			$settings['page-exhibitions-exhibition'], $exhibitor_permalink);
		if( $rc['stat'] != 'ok' ) {
			return array('stat'=>'404', 'err'=>array('pkg'=>'ciniki', 'code'=>'1290', 'msg'=>"I'm sorry, but we don't have any record of that exhibitor.", 'err'=>$rc['err']));;
		}
		$participant = $rc['participant'];
		$page_title = $participant['name'];
		$page_content .= "<article class='page'>\n"
